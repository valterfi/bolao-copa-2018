...
public class MaisBolao {
	...
	public List<Classificacao> retornarClassificacao() {
		...
		WebDriver webDriver = iniciarWebDriver();
		
		try {
		
			List<HashMap<String, WebElement>> table = pegarTableSite(webDriver);
			if(table != null) {
	...
	private WebDriver iniciarWebDriver() {
	...
	private List<HashMap<String, WebElement>> pegarTableSite(WebDriver webDriver) {
